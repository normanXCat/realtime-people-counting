...
| Lot 4 — seuils BoT-SORT (**avancé**) | `docs/lot_4_seuils_botsort.md` | **accepté** le 2026-09-21, avec la réserve de non-régression de `rare_occ2` (`docs/correctif_occlusion.md` §15, valeurs appliquées aux §15.13 et §15.15). `new_track_thresh` 0,7 → **0,60** `PROVISOIRE` ; **complément du 2026-09-21 : `track_high_thresh` 0,4 → 0,30** `PROVISOIRE` (0,25 écarté) — erreur visible 8 → 7, `TECHNICAL_ID_CHANGED` 14 → 6 ; réserve : identités provisoires 22 → 29 et doublons 4 → 8, à traiter au lot 3 (§15.15.4) (0,55 et 0,45 mesurés, écartés : doublons ×2 et ×4 pour un gain marginal) ; `match_thresh` 0,75 **écarté** ; `proximity_thresh` 0,8 **non appliqué**, à reprendre après le lot 3 ; filtre d'inclusion **écarté** (§15.12). `erreur_comptage_max_visible` **9 → 8** ; `rare_occ2` : 5 personnes réelles vues sur 5 au lieu de 4. **Réserve** : non-régression `rare_occ2` non tenue (identités 7 → 10, doublons 1 → 7), acceptée en connaissance de cause (§15.13.3) | `d96b640`, `8e46391`, `b097bce`, puis §15.15 | 2026-09-21 |
| Version E — seuils tracker + NMS (**hors plan**, sur décision) | `docs/correctif_occlusion.md` §18.1 | **appliquée** le 2026-09-22 : `new_track_thresh` 0,60 → **0,70**, `track_high_thresh` 0,30 → **0,40**, `nms_iou` 0,55 → **0,45**, tous `PROVISOIRE`. Sur la ligne inclinée `0.383,1.0,0.56,0.55` : `operational` 17 → **12 contre 12**, 0 `NEW` parasite ; `visible_count` 7/95 → **8/146** (gain du lot 4 abandonné). **Réserve** : `rare_occ2` régresse (`visible_count` max 5 → 4, `operational` 5 → 4). Étiquette `soutenance-finale` | voir `git log` | 2026-09-22 |
| Variante E2 — `proximity_thresh` (**hors plan**, sur décision) | `docs/correctif_occlusion.md` §20 | **appliquée** le 2026-09-23 : `proximity_thresh` 0,5 → **0,80** `PROVISOIRE`, seul changement. `fort_occ4` (ligne de référence) : inversions 12 → **10** (reprises d'une piste perdue inchangées, 4), `operational` 12 contre 12, aucun `NEW`/OUT parasite ni rattachement faux ; `visible_count` max 8 → 9. `rare_occ2` inchangé. FPS 3,97 / 4,08. `track_high_thresh` 0,30 (+ rayon de report 1,0) **écarté** : OUT fantôme à s14. Étiquette `soutenance-finale2` | voir `git log` | 2026-09-23 |
| Post-Occlusion Recovery, étape 1 (**hors plan**, sur instruction) | `docs/correctif_occlusion.md` §21 | **mesurée, désactivée par défaut** (`post_occlusion_recovery.enabled: false`) : détections YOLO brutes avant tracker rattachées à une identité `OCCULTEE` (OSNet ≥ 0,66, écart 0,10, 2 images), visibilité seule. `fort_occ4` : `visible_count` 9/148 → 8/142, `operational` 12 exact, aucun IN/OUT/NEW parasite ; **mais** 4 récupérations fausses sur 7 jugeables, inversions 10 → 11, OSNet 160 → 2 367 calculs, FPS 3,87 → 3,08. Non recommandée en l'état. Étiquette `soutenance-recovery1` | voir `git log` | 2026-09-23 |
| Correction d'inversions par affectation globale (**hors plan**, sur instruction) | `docs/correctif_occlusion.md` §22 | **appliquée** le 2026-09-23, portée de la branche du binôme : groupes de 3+ boîtes qui se touchent résolus par algorithme hongrois, paires inchangées, repli par paires sans `scipy`, descripteurs `on_demand` non forcés, aucun réglage modifié. Relevés `fort_occ4` et `rare_occ2` **identiques** à `soutenance-finale2` (0 `IDENTITY_SWAP_CORRECTED` avant comme après, inversions 10) : **non concluant** sur ce corpus. Étiquette `soutenance-finale3` | voir `git log` | 2026-09-23 |
...
| Interface web avec calibration (**hors plan**, sur instruction) | `docs/correctif_occlusion.md` §27 | **appliquée** le 2026-09-23 : `--web` sans aucune fenêtre OpenCV, écoute 127.0.0.1 uniquement ; calibration dans le navigateur (tracer / sans ligne, droite prolongée, côté teinté, même validation), puis vue de démonstration. Comptage inchangé : `fort_occ4` ligne de référence identique avec et sans `--web`. Port sondé avant démarrage (Windows). Étiquette `soutenance-web2` | voir `git log` | 2026-09-23 |
| Interface web : dessin partagé, lenteur, relecture (**hors plan**, sur instruction) | `docs/correctif_occlusion.md` §28 | **appliquée** le 2026-09-24 : ligne, zone morte et flèche dessinées par une seule fonction OpenCV (fenêtre, web, relecture ; identité au pixel testée) ; calibration web redessinée par le serveur ; boucle active du flux corrigée (validation → 1re image **600,9 s → 5,1 s**, 0,4 s après 20 s de tracé), modèles préchargés pendant la calibration, JPEG 960 px q75 ; `--replay <session>` (+ `dead_zone.jsonl`). Comptage inchangé : `fort_occ4` identique avec et sans `--web`. Étiquette `soutenance-web4` | voir `git log` | 2026-09-24 |

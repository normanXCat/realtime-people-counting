...
| Lot 4 — seuils BoT-SORT (**avancé**) | `docs/lot_4_seuils_botsort.md` | **mesuré, en attente d'acceptation** (`docs/correctif_occlusion.md` §15). `new_track_thresh` 0,7 → **0,45** `PROVISOIRE` ; `match_thresh` 0,75 **écarté** ; `proximity_thresh` 0,8 **non appliqué**, à reprendre après le lot 3. `erreur_comptage_max_visible` **9 → 7** (effondrement de fin de `fort_occ4` levé). **Non-régression `rare_occ2` non tenue** : identités 7 → 12, doublons emboîtés visibles. Décision attendue (§15.9) | ce commit | 2026-09-21 |
